...
(function($ , undefined) {
	...
	var app_File_Input = function(element , settings) {
		var self = this;
		this.settings = $.extend({}, $.fn.app_file_input.defaults, settings);
		...
		this.$element.on('change.app_inner_call', function(e , app_inner_call) {
			if (app_inner_call === true) return;//this change event is called from above drop event
			return handle_on_change.call(self);
		});

		this.$element.wrap('<div class="app-file-input" />');

		this.apply_settings();
	}
	app_File_Input.error = {
		'FILE_LOAD_FAILED' : 1,
		'IMAGE_LOAD_FAILED' : 2,
		'THUMBNAIL_FAILED' : 3
	};
	...
	app_File_Input.prototype.apply_settings = function() {
		var self = this;
		...
		if (this.well_style) this.$element.parent().addClass('app-file-multiple');
		 else this.$element.parent().removeClass('app-file-multiple');
		...
		if (remove_btn) this.$label.next('a').on(app.click_event, function() {
	...
	app_File_Input.prototype.show_file_list = function($files) {
		var files = typeof $files === 'undefined' ? this.$element.data('app_input_files') : $files;
	...
	app_File_Input.prototype.reset_input = function() {
	  this.$label.attr({'data-title': this.settings.btn_choose, 'class': 'file-label'})
		...
		if (this.$element.data('app_input_files')) {
			this.$element.removeData('app_input_files');
			this.$element.removeData('app_input_method');
		}
	...
	app_File_Input.prototype.reset_input_field = function() {
		//http://stackoverflow.com/questions/1043957/clearing-input-type-file-using-jquery/13351234#13351234
		this.$element.wrap('<form>').closest('form').get(0).reset();
		this.$element.unwrap();
	}

	app_File_Input.prototype.enable_reset = function(can_reset) {
		this.can_reset = can_reset;
	}

	app_File_Input.prototype.disable = function() {
		this.disabled = true;
		this.$element.attr('disabled', 'disabled').addClass('disabled');
	}
	app_File_Input.prototype.enable = function() {
		this.disabled = false;
		this.$element.removeAttr('disabled').removeClass('disabled');
	}

	app_File_Input.prototype.files = function() {
		return $(this).data('app_input_files') || null;
	}
	app_File_Input.prototype.method = function() {
		return $(this).data('app_input_method') || '';
	}

	app_File_Input.prototype.update_settings = function(new_settings) {
	...
	var enable_drop_functionality = function() {
		...
		$(dropbox).on('dragenter', function(e) {
			...
			self.$element.data('app_input_files', files);//save files data to be used later by user
			self.$element.data('app_input_method', 'drop');
	...
	var handle_on_change = function() {
		...
		if (!ret || ret.length == 0) {
			if (!this.$element.data('app_input_files')) this.reset_input_field();//if nothing selected before, reset because of the newly unacceptable (ret=false||length=0) selection
			return false;
		}
		...
		this.$element.data('app_input_method', 'select');


		if (files && files.length > 0) {//html5
			this.$element.data('app_input_files', files);
		}
		else {
		...
		reader.onload = function(e) {
			...
			$(img).one('load', function() {
				...
				if (thumb == null) {
					...
					$(this).remove();
					deferred.reject({code: app_File_Input.error['THUMBNAIL_FAILED']});
					return;
				}
				...
				deferred.reject({code: app_File_Input.error['IMAGE_LOAD_FAILED']});
			});

			img.src = e.target.result;
		}
		reader.onerror = function(e) {
			deferred.reject({code: app_File_Input.error['FILE_LOAD_FAILED']});
		}
	...
	///////////////////////////////////////////
	$.fn.app_file_input = function(option, value) {
		var retval;

		var $set = this.each(function() {
			var $this = $(this);
			var data = $this.data('app_file_input');
			var options = typeof option === 'object' && option;

			if (!data) $this.data('app_file_input', (data = new app_File_Input(this, options)));
...
(function($ , undefined) {
	$.fn.app_spinner = function(options) {
		...
		this.each(function() {
			...
			max = ('' + max).length;
				...
				$(this).addClass('spinner-input form-control').wrap('<div class="app-spinner">');
				var $parent_div = $(this).closest('.app-spinner').spinner(options).wrapInner("<div class='input-group'></div>");
				...
				if (on_sides)
				{
				  ...
				  .after('<div class="spinner-buttons input-group-btn">\
					...
					$parent_div.css('width' , (max * 20 + 160) + 'px');
				}
				else {
					...
					if ('ontouchend' in document || options.touch_spinner) {
						...
						$parent_div.css('width' , (max * 20 + 110) + 'px');
					}
				}
...
(function($ , undefined) {
	$.fn.app_wizard = function(options) {
		...
		this.each(function() {
			...
			$wizard.$prevBtn = buttons.find('.btn-prev').eq(0).on(app.click_event, function() {
				$this.wizard('previous');
			}).attr('disabled', 'disabled');
			$wizard.$nextBtn = buttons.find('.btn-next').eq(0).on(app.click_event, function() {
				$this.wizard('next');
			}).removeAttr('disabled');
...
$.fn.app_spinner_toggle = function(state) {
	if (state == 'disable') {
		this
		.attr('disabled', 'disabled')
		.siblings('.spinner-buttons')
		.find('button')
		.attr('disabled', 'disabled');
	}
	if (state == 'enable') {
...
(function($ , undefined) {
	$.fn.app_colorpicker = function(options) {
		...
		this.each(function() {
			...
			}).end().on('change.app_inner_call', function() {
			...
			.next().find('.dropdown-menu').on(app.click_event, function(e) {
				var a = $(e.target);
...
(function($ , undefined) {
	$.fn.app_wysiwyg = function($options , undefined) {
		var options = $.extend({
			speech_button: true,
			wysiwyg: {}
        }, $options);
		...
		var button_defaults =
		{
			'font' : {
				...
				icon: 'icon-font',
				title: $.i18n.prop('appElem.Font')
			},
			'fontSize' : {
				values: {5: 'Huge', 3: 'Normal', 1: 'Small'},
				icon: 'icon-text-height',
				title: $.i18n.prop('appElem.FontSize')
			},
			'bold' : {
				icon: 'icon-bold',
				title: $.i18n.prop('appElem.Bold')
			},
			'italic' : {
				icon: 'icon-italic',
				title: $.i18n.prop('appElem.Italic')
			},
			'strikethrough' : {
				icon: 'icon-strikethrough',
				title: $.i18n.prop('appElem.Strike')
			},
			'underline' : {
				icon: 'icon-underline',
				title: $.i18n.prop('appElem.Under')
			},
			'insertunorderedlist' : {
				icon: 'icon-list-ul',
				title: $.i18n.prop('appElem.Bullet')
			},
			'insertorderedlist' : {
				icon: 'icon-list-ol',
				title: $.i18n.prop('appElem.NumberList')
			},
			'outdent' : {
				icon: 'icon-indent-left',
				title: $.i18n.prop('appElem.Reduce')
			},
			'indent' : {
				icon: 'icon-indent-right',
				title: $.i18n.prop('appElem.Indent')
			},
			'justifyleft' : {
				icon: 'icon-align-left',
				title: $.i18n.prop('appElem.AlignLeft')
			},
			'justifycenter' : {
				icon: 'icon-align-center',
				title: $.i18n.prop('appElem.Center')
			},
			'justifyright' : {
				icon: 'icon-align-right',
				title: $.i18n.prop('appElem.AlignRight')
			},
			'justifyfull' : {
				icon: 'icon-align-justify',
				title: $.i18n.prop('appElem.Justify')
			},
			'createLink' : {
				icon: 'icon-link',
				title: $.i18n.prop('appElem.Link'),
				button_text: $.i18n.prop('appElem.Add'),
				placeholder: 'URL',
				button_class: 'btn-primary'
			},
			'unlink' : {
				icon: 'icon-unlink',
				title: $.i18n.prop('appElem.RemoveHyperLink')
			},
			'insertImage' : {
				icon: 'icon-picture',
				title: $.i18n.prop('appElem.InsertPic'),
				button_text: '<i class="icon-file"></i>' + $.i18n.prop('appElem.ChooseImg'),
			...
			'foreColor' : {
				values: color_values,
				title: $.i18n.prop('appElem.ChangeColor')
			},
			'backColor' : {
				values: color_values,
				title: $.i18n.prop('appElem.ChangeBkgColor')
			},
			'undo' : {
				icon: 'icon-undo',
				title: $.i18n.prop('app.ElemUndo')
			},
			'redo' : {
				icon: 'icon-repeat',
				title: $.i18n.prop('appElem.Redo')
			},
			'viewSource' : {
				icon: 'icon-code',
				title: $.i18n.prop('appElem.ViewSource')
			}
		};
		...
		this.each(function() {
			...
			toolbar.find('.dropdown-menu input:not([type=file])').on(app.click_event, function() {return false})
			...
			toolbar.find('input[type=file]').prev().on(app.click_event, function(e) {
				$(this).next().click();
			});
			toolbar.find('.wysiwyg_colorpicker').each(function() {
				$(this).app_colorpicker({pull_right: true}).change(function() {
